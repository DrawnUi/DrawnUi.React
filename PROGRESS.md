...
7. ~~`LoadMore`, structure-preserving inserts~~ done; `ItemsSourceWindow` judged unnecessary (see SKIPPED) → `SkiaEditor`/text input, ~~`SkiaSlider`/`SkiaSwitch`/`SkiaCheckbox`~~ done, ~~`SkiaCarousel`/`SkiaDrawer`~~ done, ~~`SkiaLottie`/`SkiaGif`~~ done → `SkiaEditor`, `SkiaShell` popups/modals, `SkiaSprite`.
